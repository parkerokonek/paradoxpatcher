use paradoxmerger::configs::{fetch_user_configs, parse_user_config, ArgOptions};
use paradoxmerger::{
    auto_merge, extract_all_files, files_in_vanilla, generate_enabled_mod_list,
    write_mod_desc_to_folder, ModInfo, ModPack,
};
...
fn main() {
    ...
    let mod_pack = merger
        .mod_pack_from_enabled(true)
        .expect("Could not generate mod list.");
    ...
    if !args.dry_run {
        ...
        if let Ok(num_good) = aout {
            ...
            if !mod_pack.list_conflicts().is_empty() {
                ...
                println!(
                    "Unmerged mod files output to folder {}_bad",
                    args.folder_name()
                );
            } else {
