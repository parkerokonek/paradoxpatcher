...
fn diff_linemode_nway(
    base_text: &str,
    modified_text: &[String],
) -> (HashMap<char, String>, Vec<Vec<Diff>>, String) {
...
mod test {
    ...
    fn test_diff_two_line_changes() {
        ...
        assert_eq!(
            diff_single_conflict(&source, &[change_tier, change_cash], true),
            Some(both_changes)
        );
    }
    ...
    fn test_diff_two_removals() {
        ...
        assert_eq!(
            diff_single_conflict(&source, &[no_tier, no_cash], true),
            Some(both_changes)
        );
    }
    ...
    fn test_diff_remove_first() {
        ...
        assert_eq!(
            diff_single_conflict(&source, &[no_tier, change_cash], true),
            Some(both_changes)
        );
    }
    ...
    fn test_two_conflicting_changes() {
        ...
        assert_eq!(
            diff_single_conflict(&source, &[change_tier_1, change_tier_2], false),
            None
        );
    }
    ...
    fn test_three_conflicting_changes() {
        ...
        assert_eq!(
            diff_single_conflict(
                &source,
                &[change_tier_1, change_tier_2, change_tier_3],
                false
            ),
            None
        );
    ...
    fn test_conflicting_character_changes() {
        ...
        assert_eq!(
            diff_single_conflict(&source, &[source_a.clone(), source_o.clone()], false),
            diff_single_conflict(&source, &[source_o, source_a], false),
            "Result changes based off of the order of the changed files."
        );
    }
    ...
    fn test_adding_many_lines() {
        ...
        assert_eq!(
            diff_single_conflict(
                source,
                &[big_change.clone(), small_change_later.clone()],
                true
            ),
            Some(result.clone())
        );
        assert_eq!(
            diff_single_conflict(source, &[small_change_later, big_change], true),
            Some(result)
        );
    }
