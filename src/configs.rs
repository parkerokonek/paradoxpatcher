...
impl Default for MergerSettings {
    fn default() -> Self {
        let user_path = ProjectDirs::from("com", "Parker Okonek", "Paradox Merger")
            .expect("Something went wrong in reading the user dirs.");
        //TODO: i dunno
        ...
        let patch_path = String::from(
            files::relative_folder_path(Path::new(""), Path::new(""))
                .unwrap_or_default()
                .to_string_lossy(),
        );
...
impl MergerSettings {
    ...
    ) -> Result<MergerSettings, Box<dyn std::error::Error>> {
...
impl From<TomlConfigItem> for ConfigOptions {
    fn from((game_id, config_info): TomlConfigItem) -> Self {
        let valid_paths: Vec<PathBuf> = config_info
            .valid_paths
            .iter()
...
impl From<&ConfigOptions> for TomlConfigItem {
    fn from(config_option: &ConfigOptions) -> Self {
        let valid_paths: Vec<String> = config_option
            .valid_paths
            .iter()
...
impl ConfigOptions {
    pub fn new(
        ...
        ConfigOptions {
            ...
            mod_path,
            data_path,
            valid_paths: valid_paths.to_vec(),
            valid_extensions: valid_extensions.to_vec(),
            no_transcode: no_transcode.to_vec(),
            new_launcher,
        }
    }
    ...
    pub fn parse_user_config(
        arguments: &MergerSettings,
        defaults: bool,
    ) -> Result<ConfigOptions, Box<dyn std::error::Error>> {
        ...
        if arguments.game_id.is_empty() {
            match configs.into_iter().next() {
                None => Err(Box::new(std::io::Error::new(
                    ...
                    "No games found in configuration file.",
                ))),
                Some(value) => Ok(value),
            }
        } else {
            match configs
                .into_iter()
                .find(|x| x.game_name == arguments.game_id)
            {
                Some(conf) => Ok(conf),
                None => Err(Box::new(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    "Game not found in configuration file.",
                ))),
            }
        }
    }
    ...
    fn parse_configs(config_path: &Path) -> Result<Vec<ConfigOptions>, std::io::Error> {
        ...
        if let Ok(mut file_ok) = config_file {
            ...
            if let Ok(_e) = err {
                let configs_untyped: toml::value::Table =
                    toml::from_str(&contents).expect("Configuration file could not be read!");
                let mut game_configs = Vec::new();

                for (game_id, config_untyped) in configs_untyped {
                    let config_string = config_untyped.to_string();
                    let config: ConfigListItem = toml::from_str(&config_string)
                        .expect("Malformed configuration for game found.");
        ...
        Err(std::io::Error::new(
            std::io::ErrorKind::Other,
            "This is a config parsing error that should never appear.",
        ))
    }

    pub fn fetch_user_configs(
        defaults: bool,
    ) -> Result<Vec<ConfigOptions>, Box<dyn std::error::Error>> {
        let user_path = ProjectDirs::from("com", "Parker Okonek", "Paradox Merger")
            .expect("Something went wrong in reading the user dirs.");
        let _e = fs::create_dir_all(user_path.config_dir())?;
        let config_path = user_path.config_dir().join("merger.toml");
    ...
    fn generate_default_configs() -> Vec<ConfigOptions> {
        ...
        if let Some(path) = game_paths.get("CK3") {
            let ck3_config = ConfigOptions::new_with_str(
                "CK3".to_owned(),
...
fn get_default_steamapps_dir() -> PathBuf {
    ...
    if cfg!(windows) {
        ...
        home_base
            .home_dir()
            .join(r#"Library/Application Support/Steam/steamapps/common"#)
    // Otherwise, assume Linux
    } else {
        let home_base = BaseDirs::new().expect("Something went wrong in reading the base dirs.");
        home_base
            .home_dir()
            .join(r#".steam/steam/steamapps/common"#)
    }
}

fn get_all_steam_library_folders() -> Vec<PathBuf> {
    let mut library_folders = vec![get_default_steamapps_dir()];
    let steamapps_dir = library_folders[0]
        .parent()
        .expect("Something went horribly wrong with getting the default steamapps directory.");

    let extra_folders = files::fgrep(
...
fn get_user_game_data_dir(folder_name: Option<&str>, new_launcher: bool) -> PathBuf {
    ...
    //Otherwise, assume Linux
        ...
        home_base
            .home_dir()
            .join(".local/share/Paradox Interactive")
    } else {
